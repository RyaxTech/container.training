...
_cmd_kube() {
	...
	sudo kubeadm init --token \$(cat /tmp/token) --ignore-preflight-errors=SystemVerification
    fi"
    ...
    if ! grep -q node1 /tmp/node && [ ! -f /etc/kubernetes/kubelet.conf ]; then
        ...
        sudo kubeadm join --discovery-token-unsafe-skip-ca-verification --token \$TOKEN node1:6443 --ignore-preflight-errors=cri  --ignore-preflight-errors=SystemVerification
    fi"
